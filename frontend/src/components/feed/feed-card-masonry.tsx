...
export function FeedCardMasonry({
  ...
  const columns = React.useMemo(() => {
    const bucketCount = Math.max(FEED_MASONRY_DEFAULT_COLUMN_COUNT, columnCount);
  ...
  return (
    ...
    >
      {columns.map((columnItems, columnIndex) => (
        <div key={`feed-column-${String(columnIndex)}`} className="flex flex-col gap-6">
          {columnItems.map((item, itemIndex) => (
            <React.Fragment
              key={
                React.isValidElement(item) && item.key != null
                  ? item.key
                  : `${String(columnIndex)}-${String(itemIndex)}`
              }
            >
              {item}
            </React.Fragment>
