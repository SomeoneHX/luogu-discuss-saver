...
- **龙王榜**：近 20 天活跃用户排行
- **用户页**：用户快照信息、发布/回复时间线
- **按需抓取**：未收录/已删除的讨论显示「未找到」页（可点其中的「尝试保存该讨论」按钮加入抓取队列）；已归档帖子用右侧面板的「更新帖子」手动刷新
- **未找到页**：讨论、用户、文章、云剪贴板各自的未找到文案与图标移植自 luogu-archive（文章/云剪贴板本项目不归档，故不提供入队按钮）
- **自动追更（低频率 + 随机化）**：Worker cron 每小时唤醒一次，掷骰（默认 1/3）通过后入队一条带 0\~30 分钟随机延迟的发现任务；发现轮**按版块定向**读取讨论列表（`DISCOVERY_FORUMS`，默认学术版 / 站务版 / 工单反馈版）、把置顶帖降级排到最后（不跳过：置顶里常有新通告帖；洛谷列表置顶在前，若不降级会永远占满单轮名额），仅对「洛谷回复数 − 已归档数 ≥ 阈值」且超过按帖冷却的帖子入队，单轮最多 5 个。北京时间 0\~7 点不执行（`DISCOVERY_*` 变量可全部调整）
...
| —    | Worker cron `0 * * * *` → 发现任务 | 自动追更：掷骰 + 随机延迟 + 版块定向 + 置顶降级/delta/冷却闸门（不在 Pages 暴露 HTTP 入口） |
...
- **自动发现（低频 + 随机化）**：Worker 每小时整点唤醒一次，掷骰（`DISCOVERY_PROBABILITY`，默认 1/3）通过后入队一条带 0~30 分钟随机延迟的发现任务；北京时间 0~7 点不执行；
- **按帖冷却**：`CRAWL_COOLDOWN_SECONDS`（默认 300s）限制手动触发频率；
- **发现轮版块范围**：`DISCOVERY_FORUMS`（默认 `academics,siteaffairs,service`）逐版块取列表；留空即回到「抓混合列表」；
- **题目讨论无法自动归档**：洛谷的题目讨论**只能发在具体题目下**（`?forum=problem` 的「题目总榜」只是聚合视图，不能单独发帖），且账号**对该题目有提交记录**才能查看，否则一律 403、任务被丢弃（详见「未找到页」文案）。因此发现轮默认不扫题目讨论区——混合列表里这类帖子刷新最勤，会长期霸占单轮名额却注定失败。手工把链接丢进「尝试保存该讨论」仍可逐个尝试。
