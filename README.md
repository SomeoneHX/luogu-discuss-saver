...
- **自动追更（低频率 + 随机化）**：Worker cron 每小时唤醒一次，掷骰（默认 1/3）通过后入队一条带 0\~30 分钟随机延迟的发现任务；发现轮读取讨论列表、把置顶帖降级排到最后（不跳过：置顶里常有新通告帖；洛谷列表置顶在前，若不降级会永远占满单轮名额），仅对「洛谷回复数 − 已归档数 ≥ 阈值」且超过按帖冷却的帖子入队，单轮最多 5 个。北京时间 0\~7 点不执行（`DISCOVERY_*` 变量可全部调整）
...
- **题目讨论版无法归档**：洛谷要求账号对相应题目有提交记录才能查看此类讨论，抓取会被 403 拒绝，任务随即被丢弃（详见「未找到页」文案）。
